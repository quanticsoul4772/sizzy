...
devharness is not a general agent framework. It is a specific, opinionated harness: four roles with structurally-enforced tool boundaries drive an event-sourced loop — research → spec → plan → write → verify → review → integrate — where every consequential step is earned, not asserted. A task is `completed` only when a declared verifier passes **and** a fresh-context reviewer certifies it (two separate checks). Gates fail closed. Retro proposals never auto-apply; an operator approves or rejects each one. Core safety gates cannot be weakened by the learning loop. These are invariants with tests, not prompt requests.
...
This harness does real, billable, externally-visible work. Know what you are launching:
...
```bash
# Python runtime (editable install with test extras + the console TUI)
...
Run the test suite — the "see it work" path; the acceptance tests drive the full loop end-to-end:

```bash
pytest tests/runtime -q -n auto             # ~1600 tests (from the repo root; parallel-safe)
pytest tests/specledger tests/jqlite tests/csvlite -q   # the three in-repo projects' own suites (~646)
cd sidecar && cargo test
cd dashboard && npm run check
```

> Note: the root `pyproject.toml` belongs to the bundled **jqlite** subproject (dist name
> `devharness-jqlite`) — `pip install .` at the repo root installs the jqlite demo CLI, not the
> harness. The harness runtime installs from `runtime/`.
...
(`scripts/dev_stack.sh` automates this but is Windows-shaped — `taskkill`/`netstat` teardown; on
Linux/macOS run the two commands above directly.)

## What runs without the private pieces

The write loop depends on a logged-in Claude Code CLI and two operator-local MCP servers
(`parallax`, `mcp-reasoning` — separate repositories, not bundled). Everything else works from this
repo alone; here is the boundary, so you meet it here instead of as a runtime crash:

| Needs | What you can run |
|---|---|
| **Nothing beyond this repo** | The full test matrix (CI runs it with no servers and no Claude auth), boot checks + migrations, `devharness sweep <store>` (read-only invariant monitor), the console's read-only snapshot (`python -m devharness.console status`), the web panel's read surfaces, the sidecar + dashboard over an existing event store, the operator event CLIs (`sign`/`answer`/`retro`/`memory`/`prune`/…), and the retro learning spine in T0-only mode (`run_maintenance.py --no-llm`) |
| **Claude Code installed + logged in** | Dispatching any role's SDK session (the drivers bill through your claude.ai login; a stray `ANTHROPIC_API_KEY` is cleared at startup) |
| **Both MCP servers wired** | The full write loop: the research interview (parallax `elicit`), the verifier's independent parallax axes, and director decomposition quality. Without them these paths fail closed naming the missing server. **No private servers? The repo bundles a substitute** — `python -m devharness.advisory` (advisory-lite: single-pass LLM verification, not parallax's multi-pass ensemble; restores feature/OSS completion) |

Wiring advisory-lite is one command — `python -m devharness init` writes the config and prints the
env lines; **`docs/first-build.md`** is the worked walkthrough from clone to a finished artifact.
To wire your own MCP servers instead, see **`docs/local-mcp-setup.md`** — including
`DEVHARNESS_MCP_CONFIG` for per-repo config without touching `~/.claude.json`, and the minimal
tool surface a substitute server must provide.
...
docs/         the first-build walkthrough + operator-console guide + local MCP setup
...
- **`CLAUDE.md`** — agent/operator context: architecture-at-a-glance, the rollout ladder, conventions, and current state. The fastest orientation.
- **`devharness-spec.md`** — the canonical specification: governing layer, architecture, event-sourced spine, sub-systems §S1–S9, the 18 invariants, acceptance criteria, rollout, open questions (all resolved). The source of truth; when code disagrees with the spec, the spec was wrong (current rev in the file's own Status line).
...
- **`docs/first-build.md`** — the worked walkthrough: clone → `python -m devharness init` → console → a finished artifact, with what to expect at each step.
...
- **`docs/local-mcp-setup.md`** — wiring the two MCP servers (or your own substitutes) on a local machine: the `mcpServers` config shape, `DEVHARNESS_MCP_CONFIG`, and the minimal tool surface a substitute must provide.
