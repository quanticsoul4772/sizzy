...
The console holds the §S7 operator review with a human in the seat (no LLM agent): ``accept`` /
``reject`` press the SAME accept/reject decision the ``devharness retro approve/reject`` CLI drives,
through the canonical ``retro.approval`` operation. The decision is recorded as the
operator-attributed ``candidate_reviewed`` event via ``EventBus.emit_sync`` — the console writes no
event store or projection directly — and SC-2 (no auto-apply), Inv 11, and Inv 12 are preserved
because the canonical operation is used unchanged.
"""
...
from devharness.console import ConsoleTaskDecision, UnknownQueue
from devharness.console.app import ConsoleApp
from devharness.console.task_decision import CandidateNotFound, EmptyRejectionReason
...
def test_task_decision_returns_bound_action():
    app = _app()
    assert isinstance(app.task_decision(operator="ada"), ConsoleTaskDecision)


def test_list_pending_surfaces_both_queues():
    app = _app()
    _seed_candidates(app)
    rows = app.task_decision(operator="ada").list_pending(queue="all")
    assert {r["queue"] for r in rows} == {"antibody", "gate-change"}
    assert app.task_decision().list_pending(queue="antibody")[0]["detail"] == "leak the token"


def test_accept_approves_the_antibody_candidate():
    app = _app()
    ab, _ = _seed_candidates(app)
    app.task_decision(operator="ada").accept("antibody", ab)
...
def test_accept_publishes_the_antibody_into_the_active_library():
    app = _app()
    ab, _ = _seed_candidates(app)
    new_id = app.task_decision(operator="ada").accept("antibody", ab)
    # accept returns the published antibody row id and the pattern is now active
...
def test_accept_records_an_operator_attributed_event():
    app = _app()
    ab, _ = _seed_candidates(app)
    app.task_decision(operator="ada").accept("antibody", ab)
...
def test_reject_marks_the_candidate_rejected_with_operator_and_reason():
    app = _app()
    ab, _ = _seed_candidates(app)
    returned = app.task_decision(operator="ada").reject("antibody", ab, "false positive")
    assert returned == ab
...
def test_reject_a_gate_change_candidate():
    ...
    app.task_decision(operator="ada").reject("gate-change", gc, "not worth tightening")
    state = app.conn.execute(
...
def test_reject_requires_a_reason():
    ...
    with pytest.raises(EmptyRejectionReason):
        app.task_decision(operator="ada").reject("antibody", ab, "   ")
...
def test_unknown_queue_is_refused():
    ...
    with pytest.raises(UnknownQueue):
        app.task_decision(operator="ada").accept("nope", 1)
    with pytest.raises(UnknownQueue):
        app.task_decision(operator="ada").reject("nope", 1, "a reason")


def test_unknown_candidate_is_refused():
    app = _app()
    with pytest.raises(CandidateNotFound):
        app.task_decision(operator="ada").accept("antibody", 999)


def test_per_call_operator_overrides_instance_default():
    app = _app()
    ab, _ = _seed_candidates(app)
    app.task_decision(operator="ada").accept("antibody", ab, operator="grace")
    reviewed = [e for e in _events(app.conn, "candidate_reviewed") if e["candidate_row_id"] == ab]
...
def test_now_millis_seam_stamps_the_decision():
    app = _app()
    ab, _ = _seed_candidates(app)
    action = ConsoleTaskDecision(app.conn, app.writer, operator="ada", now_millis=lambda: 4242)
...
def test_decision_goes_through_the_event_bus_not_a_raw_write():
    ...
    app.task_decision(operator="ada").reject("antibody", ab, "false positive")
