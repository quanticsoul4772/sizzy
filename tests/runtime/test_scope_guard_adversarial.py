"""B4.2: scope_guard adversarial — known-bad denies; a weakened fixture is caught."""
...
_WEAK_NAME = "b42_weak_scope_guard"
...
def test_known_bad_denies():
    conn, bus = _setup()
    result = run_probe(PROBES["loc_over_limit"], conn, bus, now_millis=lambda: 5)
...
def test_weakened_gate_caught():
    ...
    probe = KnownBadProbe(probe_name="b42_weak_loc_over_limit", target_gate=_WEAK_NAME, context_factory=lambda: {'diff_content': chr(10).join('+l' for _ in range(501))})
