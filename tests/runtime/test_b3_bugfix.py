"""B3.3: full bugfix-class write loop on a synthetic existing repo with a known bug."""
...
import devharness.verifier.builtin  # noqa: F401  (registers bugfix_regression)
...
def _run(tmp_path, fix_body):
    ...
    def write_hook(editor, shell, test_runner):
        editor.write_file("app.py", fix_body, predicted_success=0.9)

    director = DirectorRole.spawn(conn=conn, correlation_id=CID, reasoning=_reasoning(), event_bus=bus, now_millis=lambda: 1)
    tasks = [{"task_class": "bugfix", "description": "foo must return 42", "regression_test_ref": "tests/test_bug_42.py", "scope_boundary": ["app.py"], "dependencies": []}]
    plan_id = asyncio.run(director.run(
        ...
        developer_kwargs={"base_path": str(repo), "base_ref": "bug-base", "query_fn": _noop_query(), "write_hook": write_hook},
    ))
    return repo, conn, registry, plan_id


def test_bugfix_round_trip(tmp_path):
    repo, conn, registry, plan_id = _run(tmp_path, "def foo():\n    return 42\n")
    task_id = f"{CID}-t0"
    # baseline-fails + post-passes + suite-passes -> completed, earned twice
    ...
    assert conn.execute("SELECT task_class FROM proj_developer_activity WHERE event_type='write_applied' AND task_id=?", (task_id,)).fetchone()[0] == "bugfix"
...
def test_bugfix_non_passing_rewinds_and_blocks(tmp_path):
    repo, conn, registry, plan_id = _run(tmp_path, "def foo():\n    return 41\n")  # wrong fix
    task_id = f"{CID}-t0"
    # post_should_pass axis fails -> verifier failed -> auto-rewind + reject -> blocked
...
def test_bugfix_without_verifier_refused_at_admission(tmp_path):
    conn = sqlite3.connect(":memory:")
    migrate(conn)
    registry = ProjectionRegistry()
    register_handlers(registry)
    ...
    director = DirectorRole.spawn(conn=conn, correlation_id=CID, reasoning=_reasoning(), event_bus=bus, now_millis=lambda: 1)
    # a bugfix task with NO verifier_ref (bypassing the director's auto-wiring) is refused
    task = PlannedTask(task_id="t1", task_class="bugfix", description="d", scope_boundary=["app.py"], dependencies=[], correlation_id=CID, verifier_ref=None)
    ...
    fired = [r[0] for r in conn.execute("SELECT json_extract(payload,'$.gate') FROM events WHERE event_type='gate_fired'")]
    assert "verifier_attached_gate" in fired
