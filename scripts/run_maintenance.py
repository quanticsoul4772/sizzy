...
from devharness.retro.llm_client import LLMUnavailable, make_llm_fn  # noqa: E402
...
def drive(conn, event_bus, *, llm_fn, idle_millis=_DEFAULT_IDLE_MILLIS, max_retro=10_000, now_millis=None) -> dict:
    ...
    for _ in range(max_retro):
        try:
            task_id = retro.step(conn, event_bus, now_millis=now_millis)
        except LLMUnavailable as exc:
            # The analysis never happened — no retro_run was emitted, so this terminal (and every
            # later one, including any that would take the LLM-free T0 path) stays queued for the
            # next window. Halting is the fix for the rev-0.3.57 burn: a down SDK must not consume
            # the whole queue as "analyzed, nothing found".
            print(f"[run_maintenance] retro halted: LLM unavailable — terminals left queued ({exc})")
            break
        if task_id is None:
            break
    ...
    signals_processed = []
    for _ in range(max_retro):
        sig_id = signal_retro.step(conn, event_bus, now_millis=now_millis)
        if sig_id is None:
            break
        signals_processed.append(sig_id)
    return {"retro_processed": processed, "maintenance_cycle": cycle, "adversarial_probe_ran": probe_ran,
            "loop_fault_ran": loop_fault_ran, "signals_processed": signals_processed,
...
def main() -> int:
    ...
    summary = drive(conn, bus, llm_fn=llm_fn, idle_millis=idle_millis)
